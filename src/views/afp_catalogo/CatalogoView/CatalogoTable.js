...
function catalogoMaestroFilter(array, catalogosMaestros, catalogosMaestrosSelected) {
	let catsIdSelected = [];
	catalogosMaestrosSelected.forEach((item, index) => {
		if (item) {
			catsIdSelected.push(catalogosMaestros[index].cat_id);
		}
	});

	let result = [];

	result = array.filter((arrayItem) => catsIdSelected.includes(arrayItem.cat_id));
...
const CatalogoMaestroTable = ({
	...
	const [ catalogosMaestrosSelected, setCatalogosMaestrosSelected ] = useState([]);

	useEffect(
		() => {
			setCatalogosMaestrosSelected(catalogosMaestros.map((cat) => true));
		},
		[ catalogosMaestros ]
	);
	...
	const handleSelectCatalogoMaestro = (index) => {
		catalogosMaestrosSelected[index] = !catalogosMaestrosSelected[index];
		setCatalogosMaestrosSelected([ ...catalogosMaestrosSelected ]);
	};

	const handleTodosSelect = () => {
		setCatalogosMaestrosSelected([ ...catalogosMaestrosSelected.map((_) => true) ]);
	};

	const handleNingunoSelect = () => {
		setCatalogosMaestrosSelected([ ...catalogosMaestrosSelected.map((_) => false) ]);
	};
								...
								<TableCell>
									...
									>
										<FilterForm
											...
											catalogosMaestros={catalogosMaestros}
											catalogosMaestrosSelected={catalogosMaestrosSelected}
											onSelectCatalogoMaestro={handleSelectCatalogoMaestro}
											onSort={(orderBy, orderMode) => handleRequestSort(orderBy, orderMode)}
											onTextFilterChange={(event) => handleComparatorsChange(event)}
											onTodosClick={handleTodosSelect}
											onNingunoClick={handleNingunoSelect}
										/>
									</Popover>
								</TableCell>
								<TableCell>
